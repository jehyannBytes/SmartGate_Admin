...
<div class="space-y-6">
  ...
  <div class="bg-white border border-[#1e2a5e]/30 rounded-2xl p-6 shadow-sm">
    <h2 class="text-slate-900 font-semibold mb-5">Generate New Report</h2>
    <form method="POST" action="{{ route('reports.generate') }}"
      ...
      {{-- Month --}}
      <div>
        <label class="block text-slate-500 text-xs uppercase tracking-wider mb-1.5">Month</label>
        <select name="month"
      ...
      {{-- Year --}}
      <div>
        <label class="block text-slate-500 text-xs uppercase tracking-wider mb-1.5">Year</label>
        <select name="year"
                class="bg-white border border-slate-200 rounded-xl px-4 py-2.5
                       text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-[#1e2a5e]/40">
      ...
      {{-- Department --}}
      <div>
        <label class="block text-slate-500 text-xs uppercase tracking-wider mb-1.5">Department</label>
  ...
  <div class="bg-white border border-[#1e2a5e]/30 rounded-2xl overflow-hidden shadow-sm">
    <div class="px-6 py-4 border-b border-slate-200">
      <h2 class="text-slate-900 font-semibold">Generated Reports</h2>
    </div>
    <table class="w-full">
      <thead>
        <tr class="border-b border-slate-200 bg-slate-50">
          <th class="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Period</th>
          <th class="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Department</th>
          <th class="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Generated By</th>
          <th class="text-left px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Generated At</th>
      ...
      <tbody class="divide-y divide-slate-100">
        @forelse($reports as $report)
          <tr class="hover:bg-slate-50 transition-colors">
            ...
            <td class="px-6 py-4">
              <span class="text-slate-600 text-sm">{{ $report->department }}</span>
            </td>
            <td class="px-6 py-4">
              <span class="text-slate-600 text-sm">{{ $report->generatedBy?->full_name ?? 'N/A' }}</span>
            </td>
            <td class="px-6 py-4">
    ...
    @if($reports->hasPages())
      <div class="px-6 py-4 border-t border-slate-200">{{ $reports->links() }}</div>
    @endif
  </div>
</div>
@endsection