...
<div class="max-w-3xl">
  ...
  @if(session('error'))
    <div class="bg-red-500/10 border border-red-400/30 text-red-300 text-sm rounded-xl px-4 py-3 mb-5">
      {{ session('error') }}
    </div>
  @endif
  ...
  <div class="bg-white/5 border border-white/10 rounded-2xl p-8">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h3 class="text-white font-semibold text-lg">{{ $employee->full_name }}</h3>
        <p class="text-white/40 text-sm">{{ $employee->employee_code }} . {{ $employee->position }}</p>
      </div>
      <div class="text-right">
        <p class="text-white/40 text-xs">Captures</p>
        <p class="text-2xl font-bold text-[#4CAF82]">
          <span id="capture-count">{{ $existingCount }}</span><span class="text-white/30 text-base"> / {{ $minCaptures }} min</span>
        </p>
      </div>
    </div>
    ...
    <div class="relative bg-black rounded-2xl overflow-hidden aspect-video mb-4">
      ...
      <div id="camera-error" class="hidden absolute inset-0 flex items-center justify-center bg-black text-center p-6">
      ...
      <div id="face-guide" class="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div class="w-48 h-60 border-2 border-white/30 rounded-[50%]"></div>
      </div>
    ...
    <div class="flex items-center justify-between mb-6">
      <p id="capture-status" class="text-white/50 text-sm">
    ...
    <div class="flex items-center gap-3 mb-8">
      <button type="button" id="capture-btn"
              class="bg-[#4CAF82] hover:bg-[#3d9e71] text-white font-semibold px-6 py-2.5 rounded-xl text-sm transition-colors flex items-center gap-2">
      ...
      <span id="capture-hint" class="text-white/30 text-xs"></span>
    </div>

    <div>
      <p class="text-white/60 text-sm font-medium mb-3">Captured Photos</p>
      <div id="thumbnails" class="grid grid-cols-5 gap-3">
        @foreach($employee->faceCaptures as $capture)
          <div class="relative group aspect-square rounded-xl overflow-hidden border border-white/10" data-capture-id="{{ $capture->capture_id }}">
            <img src="{{ asset($capture->image_path) }}" class="w-full h-full object-cover"/>
            <button type="button" class="delete-capture-btn absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity"
      ...
      <p id="empty-hint" class="text-white/25 text-xs mt-2 @if($existingCount > 0) hidden @endif">
        No photos captured yet.
      </p>
    </div>

    <div class="flex items-center gap-3 pt-8 mt-2 border-t border-white/10">
      <form method="POST" action="{{ route('employees.enroll-face.complete', $employee) }}">
        @csrf
        <button type="submit" id="finish-btn"
                class="bg-[#4CAF82] hover:bg-[#3d9e71] disabled:bg-white/10 disabled:text-white/30 disabled:cursor-not-allowed
...
(function () {
  ...
  function addThumbnail(captureId, imageUrl) {
    ...
    div.innerHTML = `
      ...
      <button type="button" class="delete-capture-btn absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity" data-capture-id="${captureId}">
        <svg class="w-5 h-5 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
        </svg>
      </button>
