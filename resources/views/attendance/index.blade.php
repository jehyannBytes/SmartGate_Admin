...
<div class="space-y-4">
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <div class="bg-white/5 border border-white/10 rounded-2xl p-5">
      <p class="text-white/50 text-xs uppercase tracking-wider mb-2">Total Logs</p>
      <p class="text-3xl font-bold text-white">{{ $summary['total'] }}</p>
    </div>
    <div class="bg-blue-500/5 border border-blue-500/20 rounded-2xl p-5">
      <p class="text-blue-400 text-xs uppercase tracking-wider mb-2">Morning In</p>
      <p class="text-3xl font-bold text-white">{{ $summary['morning_in'] }}</p>
      <p class="text-white/30 text-xs mt-1">Out: {{ $summary['morning_out'] }}</p>
    </div>
    <div class="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-5">
      <p class="text-yellow-400 text-xs uppercase tracking-wider mb-2">Afternoon In</p>
      <p class="text-3xl font-bold text-white">{{ $summary['afternoon_in'] }}</p>
      <p class="text-white/30 text-xs mt-1">Out: {{ $summary['afternoon_out'] }}</p>
    </div>
    <div class="bg-red-500/5 border border-red-500/20 rounded-2xl p-5">
      <p class="text-red-400 text-xs uppercase tracking-wider mb-2">Unverified</p>
      <p class="text-3xl font-bold text-white">{{ $summary['unverified'] }}</p>
      <p class="text-white/30 text-xs mt-1">Pending sync: {{ $summary['pending_sync'] }}</p>
    </div>
  </div>
  ...
  <form method="GET" action="{{ route('attendance.index') }}"
    ...
    @if(request()->hasAny(['search', 'log_type', 'face_verified', 'sync_status']))
      <a href="{{ route('attendance.index') }}"
         class="text-white/40 hover:text-white text-sm transition-colors">Clear</a>
    @endif
  </form>
    ...
    <table class="w-full">
      <thead>
        <tr class="border-b border-white/10 bg-white/3">
          ...
          <th class="text-left px-6 py-4 text-xs font-semibold text-white/50 uppercase tracking-wider">Log Type</th>
          <th class="text-left px-6 py-4 text-xs font-semibold text-white/50 uppercase tracking-wider">Time</th>
          <th class="text-left px-6 py-4 text-xs font-semibold text-white/50 uppercase tracking-wider">Face</th>
          <th class="text-left px-6 py-4 text-xs font-semibold text-white/50 uppercase tracking-wider">Sync</th>
        </tr>
      </thead>
      <tbody class="divide-y divide-white/5">
        @forelse($logs as $log)
          @php
            ...
            $color = $typeColors[$log->log_type] ?? 'bg-white/10 text-white/50 border-white/10';
          @endphp
          <tr class="hover:bg-white/3 transition-colors">
            {{-- Employee --}}
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-[#4CAF82]/15 border border-[#4CAF82]/30
                ...
                <div>
                  ...
                  <p class="text-white/30 text-xs">{{ $log->employee?->position ?? '' }}</p>
                </div>
              </div>
            </td>
